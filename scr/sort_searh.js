function getArrayFromStorage() {
    ...
    for (let i = 0; i < keyNumbers; i++) {
        ...
        elm.id = keyName;
        elm.modal = row.modal;
        elm.capacity = row.capacity;
        elm.conectors = row.conectors;
        elm.size = row.size;
        elm.fastChargingSupport = row.fastChargingSupport;
...
function searchElements(){
  ...
  for (let i=0; i<searchtArr.length; i++) {
      ...
      let strM = row.modal.toLowerCase();
      let strI = row.capacity.toLowerCase();
      let strS = row.conectors.toLowerCase();
     
      
      if (regExp.test(strV) || regExp.test(strM) || regExp.test(strI) || regExp.test(strS)) {buildElementToPage(row.id, row)
                                                                        isFounded = true}
      }
  if (!isFounded) {document.getElementsByClassName("displayzone")[0].innerHTML = '<h1 style="fastChargingSupport:red; width:100%; text-align: center;" >No matches found</h1>'}
